...
TOKEN = '7210102495:AAGI-kwfFvHcML-6kRMnTHJPS_UTyCf18Ik'
...
FORWARD_CHANNEL_ID = -1002215958432
CHANNEL_ID = -1002215958432
error_channel_id = -1002215958432
...
def attack_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "*PROVIDE IP PORT SEC*", parse_mode='Markdown')
        bot.register_next_step_handler(message, process_attack_command)
    except Exception as e:
        logging.error(f"Error in attack command: {e}")
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*Attack started 💥\n\nHost: {target_ip}\nPort: {target_port}\nTime: {duration}\nAPI SERVER RUNNING....*", parse_mode='Markdown')
    except Exception as e:
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*FOR RESSELERSHIP DM:-@KRISHNAVIPOWNER*", parse_mode='Markdown')
        ...
        bot.reply_to(message, "*STRESSER SERVER RUNNING SLOW....*", parse_mode='Markdown')
